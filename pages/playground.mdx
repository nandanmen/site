...
export default ({ children }) => (
  <CaseStudy meta={meta}>
    <div>
      <figure className="relative full-width">
        <Circle className="w-40 h-40 bg-blue-400 -right-16 -top-6" />
        <div className="absolute w-full h-full bg-gray-500 rounded-lg top-2 left-2"></div>
        <video
          className="relative lg:rounded-xl"
      ...
      <CaseStudy.Content>{children}</CaseStudy.Content>
    </div>
  </CaseStudy>
)
...
Users can also input their own arguments, which change as the function signature changes.
...
<Figure caption="The evaluation of the code doesn't block the user from typing.">
